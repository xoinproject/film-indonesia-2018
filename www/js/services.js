angular.module("film_indonesia_2018.services", [])
// TODO: --|---- directive
...
.directive("runAppEmail", function(){
	return {
			...
			function runApp(event)
			{
				var EmailAddr = $attrs.email || "info@smartapp.tech";
...
.directive("runAppFacebook", function(){
	return {
			...
			function runApp(event)
			{
				var textLink = window.encodeURIComponent($attrs.link) || "http://smartapp.tech/";
				if (ionic.Platform.isIOS()){
...
.directive("runOpenURL", function(){
	...
	return {
			...
			function runApp(event)
			{
				var $href = $attrs.href || "http://smartapp.tech/";
